...
- [x] S6 导入回放测试（`tools/validate_memory_import_v0_1.py`）
- [ ] S7 Reflect 调度接入
- [x] S8 Opinion 规则增强（冲突聚类 + 极性判定 + 回放验证）
- [x] S9 Recall 质量基线（`tools/validate_recall_quality_v0_1.py` + `data/fixtures/recall-quality-v0_1/`）
- [ ] S10 发布前总检
- [ ] S11 版本产出
...
- 2026-02-24：完成 S9（Recall 质量基线）：新增 `tools/validate_recall_quality_v0_1.py`、`data/fixtures/recall-quality-v0_1/`，固化 accuracy/recall/noise 与 M→E 输入质量指标。
